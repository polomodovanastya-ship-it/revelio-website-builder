...
export const VENDOR_PROFILES: VendorProfile[] = [
  ...
  },
  {
    ...
    pros: [
      'Богатый функционал "из коробки", функционально богатая CDP с отточенными фичами на большом наборе клиентов',
      ...
      'Быстрый запуск и низкий порог входа (готовые SDK для МП, конструктор рассылок с автоматической подгрузкой товаров из фида, гибкие способы передачи данных, поддержка low-code подхода)',
      'Сильная методологическая экспертиза, качественные стратегии работы с клиентскими данными и автоматизации маркетинга',
